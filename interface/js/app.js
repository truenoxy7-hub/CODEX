(function () {
  ...
  function renderCompositionDiagnostics(snapshot) {
    ...
    elements.compositionActionsSummary.innerHTML = `<h4>ACCIONS</h4>${diagnostics.actions.length ? diagnostics.actions.map((action) => `<article class="diagnostic-action"><header><strong>${escape(diagnosticValue(action.type))}</strong><span>${escape(diagnosticValue(action.id))}</span></header><dl><div><dt>actor</dt><dd>${escape(diagnosticValue(action.actor))}</dd></div><div><dt>objectiu / receptor / company</dt><dd>${escape(diagnosticValue(action.target))}</dd></div><div><dt>defensor / oponent</dt><dd>${escape(diagnosticValue(action.opponent))}</dd></div><div><dt>espai inicial</dt><dd>${escape(diagnosticValue(action.initialSpace))}</dd></div><div><dt>espai final</dt><dd>${escape(diagnosticValue(action.finalSpace))}</dd></div>${action.crossing ? `<div><dt>primer jugador</dt><dd>${escape(diagnosticValue(action.crossing.firstActor))}</dd></div><div><dt>atac inicial</dt><dd>${escape(diagnosticValue(action.crossing.initialAttack))}</dd></div><div><dt>jugador que encreua</dt><dd>${escape(diagnosticValue(action.crossing.crossingActor))}</dd></div><div><dt>encreua respecte de</dt><dd>${escape(diagnosticValue(action.crossing.relativeTo))}</dd></div><div><dt>espai objectiu</dt><dd>${escape(diagnosticValue(action.crossing.targetSpace))}</dd></div>` : ""}<div><dt>estat origen</dt><dd>${escape(diagnosticValue(action.originState))}</dd></div><div><dt>estat destí</dt><dd>${escape(diagnosticValue(action.destinationState))}</dd></div><div><dt>autoritat</dt><dd>${escape(diagnosticValue(action.authority))}</dd></div><div><dt>status</dt><dd>${escape(diagnosticValue(action.status))}</dd></div></dl></article>`).join("") : '<p class="empty-copy">No hi ha accions actuals per inspeccionar.</p>'}`;
    const outputIds = {
